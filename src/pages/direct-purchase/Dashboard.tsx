...
import { directPurchaseApi, dashboardApi } from '@/lib/api';
...
export default function DirectPurchaseDashboard() {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const uniqueSuppliers = [...new Set(ordersData.map((order: any) => order.supplier))].filter(Boolean);
      setSuppliers(uniqueSuppliers.map(supplier => ({ name: supplier })));
