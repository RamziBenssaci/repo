...
export default function Warehouse() {
  ...
  const handleDeleteItem = async (itemId: string) => {
    ...
    try {
      setLoadingAction(true);
      const response = await warehouseApi.deleteInventoryItem(itemId);
                  ...
                  <tr key={item.id} className="border-b border-border text-right hover:bg-accent">
                    ...
                    <td className="p-2">
                      <div className="flex gap-1 justify-center">
                        ...
                        <button className="p-1.5 text-danger hover:bg-danger/10 rounded" title="حذف">
                          <Trash2 size={14} />
                        </button>
                      </div>
                    </td>
          ...
          <div className="bg-background rounded-lg w-full max-w-4xl max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleAddSubmit} className="p-6 space-y-6">
              ...
              <div className="flex gap-3 justify-end">
                ...
                <button
                  type="submit"
                  className="admin-btn-success text-sm flex items-center gap-2"
                >
                  <Save size={16} />
                  حفظ التعديلات
                </button>
